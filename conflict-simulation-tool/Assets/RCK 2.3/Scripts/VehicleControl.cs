...
public class VehicleControl : MonoBehaviour
{
    ...
    private WASP_DriveInterface wasp_interface;
    public Light controlModeLight;
    public AudioSource audio_tor;
    public AudioClip tor_manual;
    public AudioClip tor_automatic;
    public AudioClip tor_prepare;
    public GameObject path_follower;
    public float targetTime = 30.0f;
    public float switchRangeA = 10.0f;
    public float switchRangeB = 30.0f;
    private float uncertainty = 0.8f;


    public ControlMode controlMode = ControlMode.simple;

    public bool autonomousDriving = false;
    public bool pathfollowing = false;
    private bool tor_sequence = false;
    ...
    void Awake()
    {
        // random range for switch to the lane following model
        targetTime = Random.Range(switchRangeA,switchRangeB);

        if (carSetting.automaticGear) NeutralGear = false;

        wasp_interface = car.GetComponent<WASP_DriveInterface>();
        if(autonomousDriving){
            controlModeLight.color = Color.red;
        } else {
            controlModeLight.color = Color.green;
        }
    ...
    private IEnumerator CoRoutineTor(float waitTime){
        audio_tor.PlayOneShot(tor_prepare, 0.5f); 
        controlModeLight.color = Color.yellow;   
        yield return new WaitForSeconds(waitTime);
        tor_sequence = !tor_sequence;
        controlModeLight.color = Color.green;                
        audio_tor.PlayOneShot(tor_manual, 0.5f);   
        //tor_sequence = !tor_sequence;
    ...
    public void initTOR(float duration){
        // init take over request
        //Debug.Log("TOR initialized");
        autonomousDriving = !autonomousDriving;

        if(autonomousDriving){

            controlModeLight.color = Color.red;
            audio_tor.PlayOneShot(tor_automatic, 0.5f);  
            ...
            //audio_tor.PlayOneShot(tor_manual, 0.5f);
            if(duration >= 1.0f){
                StartCoroutine(CoRoutineTor(duration));
            } else {
                tor_sequence = !tor_sequence;
                controlModeLight.color = Color.green;                
                audio_tor.PlayOneShot(tor_manual, 0.5f);   
    ...
    void Update()
    {
        ...
        // manual switch manual <-> automatic control
        if(Input.GetKey(KeyCode.P)){
            // init TOR with n seconds duration
            initTOR(0.0f);
            //pathfollowing = true;
            ...
            if (controlMode == ControlMode.simple)
            {
                ...
                if (carWheels.wheels.frontWheelDrive || carWheels.wheels.backWheelDrive)
                {
                    ...
                    if(autonomousDriving){
                        ...
                        steer = Mathf.MoveTowards(steer, wasp_interface.wasp_steer, 0.2f);
                        accel = wasp_interface.wasp_accel;
                        //Debug.Log(accel);
                        Debug.Log(wasp_interface.wasp_steer);

                    /*
                    // This code first follows the map for a short time, 
                    // then switches to using the lane following model

                    if(autonomousDriving){
                        // activate pre defined path follower, then switch to the lane following model
                        ...
                        if (targetTime <= 0.0f)
                        {
                            // Lane following Model
                            if(uncertainty <= 0.9){
                                //StartCoroutine(CoRoutineTor(3.0f));
                                // create random transition
                                initTOR(Random.Range(0.5f,3.0f));
                            }
                            pathfollowing = false;
                      ...
                      */  
                        ...
                        steer = Mathf.MoveTowards(steer, Input.GetAxis("Horizontal"), 0.02f);
